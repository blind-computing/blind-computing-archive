...
. ./includes/nav.inc.sh "What Is Linux"
cat <<END-OF-CAT
<h1>What on Earth is Linux?</h1>
...
<p>  An operating system is essentially a piece of software, a program, that runs and manages the computer. In its simplest form, it displays stuff on the screen, allows you to smash that board of keys in fromt of you and is responsible for all those blue screens of death we all saw in the 90s. But seriously, an OS handles almost any task that you use a computer for in some way. Often programs can be loaded onto an operating system (they can be installed) to extend its functionality.</p>
